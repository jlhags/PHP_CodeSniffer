...
namespace PHP_CodeSniffer\Standards\Zeus\Sniffs\Classes;
...
class ScopeOpeningBraceSniff implements Sniff
{
	...
	public function process(File $phpcsFile, $stackPtr)
	{
		...
		if ($braceLine === $classLine) {
			$phpcsFile->recordMetric($stackPtr, 'scope opening brace placement', 'same line');
			$error = 'Opening brace of a %s must be on a new line';
			...
			$phpcsFile->recordMetric($stackPtr, 'scope opening brace placement', 'new line');

			if ($braceLine > ($classLine + 1)) {
				$error = 'Opening brace of a scope must be on the line following the %s declaration; found %s line(s)';
				$data  = array(
